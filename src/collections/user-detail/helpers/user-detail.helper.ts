...
export class UserDetailHelper {
	...
	public updateUserDetailBasedOnDibsEasyPayment(userDetailId: string, dibsEasyPayment: DibsEasyPayment, accessToken: AccessToken): Promise<UserDetail> {
		return new Promise((resolve, reject) => {
			this._userDetailStorage.get(userDetailId).then((userDetail: UserDetail) => {
				let updateObject = this.getUserDetailUpdateObject(dibsEasyPayment, userDetail);

				this._userDetailStorage.update(userDetailId, updateObject, {id: accessToken.sub, permission: accessToken.permission}).then((updatedUserDetail: UserDetail) => {
	...
	private getUserDetailUpdateObject(dibsEasyPayment: DibsEasyPayment, userDetail: UserDetail): any {
		...
		if (isNullOrUndefined(userDetail.address) || userDetail.address.length <= 0) {
			if (dibsShippingAddress.addressLine1) {
				userDetailUpdateObject['address'] = dibsShippingAddress.addressLine1 + ' ' + dibsShippingAddress.addressLine2;;
			}
		}
