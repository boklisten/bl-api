...
describe('UserDetailValidOperation', () => {
	...
	const userDetailValidOperation = new UserDetailValidOperation(userDetailStorage, responseHandler);
	...
	describe('#run', () => {
		...
		let resHandlerSendResponseStub = sinon.stub(responseHandler, 'sendResponse').callsFake((res: any, blApiResponse: BlapiResponse) => {

		});

		let resHandlerSendErrorResponseStub = sinon.stub(responseHandler, 'sendErrorResponse').callsFake((res: any, blError: BlError) => {
		});
		...
		it('should reject if userDetail is not found', (done) => {
			...
			userDetailValidOperation.run(blApiRequest, null, null).catch((blError: BlError) => {

				expect(resHandlerSendErrorResponseStub)
					.to.have.been.called;

				expect(blError.getMsg())
					.to.be.eql('userDetail could not be validated');

				expect(blError.errorStack[0].getMsg())
					.to.be.eql(`userDetail "notFoundUserDetail" not found`);
				done();
			})
		});
			...
			it('should send response with {valid: true}', (done) => {
				...
				userDetailValidOperation.run(blApiRequest, null, null).then(() => {
					expect(resHandlerSendResponseStub)
						.to.have.been.calledWith(null, new BlapiResponse([{valid: true}]));
		...
		context('when user detail is not valid', () => {
			...
			it('should resolve with valid false if name is not defined', (done) => {
				...
				userDetailValidOperation.run(blApiRequest, null, null).then(() => {
					expect(resHandlerSendResponseStub)
						.to.have.been
						.calledWith(null, new BlapiResponse([{valid: false, invalidFields: ['name']}]));
					done();
				})
			});
			...
			it('should resolve with valid false if address and postCode is not defined', (done) => {
				...
				userDetailValidOperation.run(blApiRequest, null, null).then(() => {
					expect(resHandlerSendResponseStub)
						.to.have.been
						.calledWith(null, new BlapiResponse([{valid: false, invalidFields: ['address', 'postCode']}]));
					done();
				})
			});
			...
			it('should resolve with valid false if postCity and phone is not defined', (done) => {
				...
				userDetailValidOperation.run(blApiRequest, null, null).then(() => {
					expect(resHandlerSendResponseStub)
						.to.have.been
						.calledWith(null, new BlapiResponse([{valid: false, invalidFields: ['postCity', 'phone']}]));
					done();
				})
			});
			...
			it('should resolve with valid false if dob is not defined', (done) => {
				...
				userDetailValidOperation.run(blApiRequest, null, null).then(() => {
					expect(resHandlerSendResponseStub)
						.to.have.been
						.calledWith(null, new BlapiResponse([{valid: false, invalidFields: ['dob']}]));
					done();
				})
			});
		});
