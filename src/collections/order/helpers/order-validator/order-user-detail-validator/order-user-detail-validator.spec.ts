...
describe('OrderUserDetailValidator', () => {
	...
	const orderUserDetailValidator = new OrderUserDetailValidator(userDetailStorage);
	let testOrder: Order;
	...
	describe('#validate', () => {
		it('should reject if userDetail is not found', (done) => {
			...
			orderUserDetailValidator.validate(testOrder).catch((err: BlError) => {
				expect(err.errorStack[0].getMsg())
					.to.be.eq('could not get userDetail');
				done();
			})
		});
		...
		it('should resolve if userDetail is valid', () => {
			return expect(orderUserDetailValidator.validate(testOrder))
				.to.be.fulfilled;
		});
	});
});