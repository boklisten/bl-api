...
export class OrderItemExtendValidator {
	...
	public async validate(branch: Branch, orderItem: OrderItem): Promise<boolean> {
		...
		try {
			...
			return Promise.reject(new BlError('unknown error, could not validate orderItem.type "extend"').store('error', e));
		}
	...
	private validateCustomerItem(branch: Branch, orderItem: OrderItem): Promise<boolean> {
		return this.customerItemStorage.get(orderItem.info.customerItem).then((customerItem: CustomerItem) => {
	...
	private checkPeriodType(orderItem: OrderItem, branch: Branch) {
		...
		throw new BlError(`orderItem.info.periodType is "${orderItem.info.periodType}" but it is not allowed by branch`);
	}
}