...
describe('OrderItemRentPeriodValidator', () => {
	...
	const orderItemRentPeriodValidator = new OrderItemRentPeriodValidator(orderStorage);
	...
	describe('#validate', () => {
		it('should reject if period is not found in branchPaymentInfo', () => {
			...
			return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100))
		...
		context('when orderItem is moved from another order', () => {
			...
			it('should reject if orderItem.amount is 0 but the movedFromOrder has not been payed for', (done) => {
				...
				orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100).then(() => {
					done(new Error('should not resolve'));
				}).catch((err: BlError) => {
					expect(err.getMsg()).to.contain('the original order has not been payed, but current orderItem.amount is "0"');
			...
			it('should reject if period is the same but orderItem.amount is not 0', (done) => {
				...
				orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, 100).then(() => {
					...
					expect(err.getMsg()).to.contain('the original order has been payed, but current orderItem.amount is "100"');
					expect(orderStorageGetStub).to.have.been.called;
					done();
				}).catch((err) => {
					done(err);
			...
			context('when new orderItem has a different period type than the original orderItem', () => {
				it('should reject if orderItem.amount is not equal to rentPrice - oldOrderItem.amount ', (done) => {
					branchPaymentInfo.rentPeriods.push({type: 'year', date: new Date(), maxNumberOfPeriods: 1, percentage: 0.5})
					orderB.payments = ['payment1']; // this means that the order is payed for
					orderStorageGetStub.withArgs('orderB').resolves(orderB);
					...
					orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice).then(() => {
						done(new Error('should not resolve'));
					}).catch((err: BlError) => {
						expect(err.getMsg()).to.contain('orderItem amount is "100" but should be "50" since the old orderItem.amount was "200"');
						expect(orderStorageGetStub).to.have.been.called;
						done();
					}).catch((err) => {
						done(err);
				...
				it('should reject if orderItem.amount is not equal to rentPrice 500 - oldOrderItem.amount 750 = -250', (done) => {
					branchPaymentInfo.rentPeriods = [
						{type: 'semester', date: new Date(), maxNumberOfPeriods: 1, percentage: 0.5}
					];
					orderB.payments = ['payment1']; // this means that the order is payed for
					orderStorageGetStub.withArgs('orderB').resolves(orderB);
					...
					orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice).then(() => {
						...
						expect(err.getMsg()).to.contain('orderItem amount is "0" but should be "-250" since the old orderItem.amount was "750"');
						expect(orderStorageGetStub).to.have.been.called;
						done();
					}).catch((err) => {
						done(err);
				...
				it('should resolve if orderItem.amount is equal to rentPrice 500 - oldOrderItem.amount 750 = -250', () => {
					...
					return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice))
						.to.be.fulfilled;
				});
			});
			...
			it('should reject if orderItem.amount is not equalt to branchPayment percentage * itemPrice', () => {
				...
				return expect(orderItemRentPeriodValidator.validate(orderItem, branchPaymentInfo, itemPrice))
					.to.be.rejectedWith(BlError, /orderItem.amount "0" is not equal to itemPrice "100" \* percentage "0.5" "50"/);
			});
