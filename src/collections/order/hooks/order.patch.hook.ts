...
export class OrderPatchHook extends Hook {
	...
	after(orderIds: string[], accessToken: AccessToken): Promise<boolean | Delivery[]> {
		...
		return this.orderStorage.get(orderIds[0]).then((order: Order) => {
			return this.orderValidator.validate(order).then(() => {
				return this.updateUserDetailsWhenOrderIsPlaced(accessToken, order);
			},(validationError: BlError) => {
				if (order.placed) {
					return this.orderStorage.update(order.id, {placed: false}, {id: accessToken.sub, permission: accessToken.permission}).then((updatedOrder: Order) => {
						return Promise.reject(new BlError('validation of patch of order failed, order.placed is set to false').add(validationError))
					}, (updateError: BlError) => {
						return Promise.reject(new BlError('could not set order.placed to false when order validation failed').add(updateError).add(validationError))
					});
					
				}
				return Promise.reject(new BlError('patch of order could not be validated'));
			});
		}, (blError: BlError) => {
			return Promise.reject(new BlError(`order "${orderIds[0]}" not found`));
		}).catch((blError: BlError) => {
			return Promise.reject(new BlError(`cold not update order on patch`).add(blError));
		})
		
	}
	
	
	private updateUserDetailsWhenOrderIsNotValidated(accessToken: AccessToken, order: Order): Promise<boolean> {
		return this.userDetailStorage.get(accessToken.details).then((userDetail: UserDetail) => {
			...
			return this.userDetailStorage.update(userDetail.id,{orders: newOrderIds}, {id: userDetail.user.id, permission: userDetail.user.permission}).then(() => {
				return true;
