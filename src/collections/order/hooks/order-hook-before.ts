	...
	validate(requestJsonBody: any): Promise<boolean> {
		return new Promise((resolve, reject) => {
		    ...
		    if (Object.prototype.toString.call(requestJsonBody) === '[object Array]') {
		    	return reject(new BlError('request is an array but should be a object').store('requestBody', requestJsonBody).code(701));
			}
			...
			try {
				...
				reject(new BlError('unkown error, request body is not valid').store('error', e).code(701));
			}
