...
export class PasswordResetNewOperation implements Operation {
	...
	constructor(passwordResetStorage?: BlDocumentStorage<PasswordReset>, localLoginHandler?: LocalLoginHandler, responseHandler?: SEResponseHandler) {
		...
		this._resHandler = (responseHandler) ? responseHandler : new SEResponseHandler();
	}

	run(blApiRequest: BlApiRequest, req?: Request, res?: Response, next?: NextFunction): Promise<boolean> {
		return new Promise((resolve, reject) => {
			if (isNullOrUndefined(blApiRequest.data) || isNullOrUndefined(blApiRequest.data['password'])) {
				return reject(new BlError('blApiRequest.data.password is null or undefined'));
			}

			let newPassword = blApiRequest.data['password'];

			if (newPassword.length < 6) {
				return reject(new BlError('blApiRequest.data.password is under length of 6'));
			}

			this._passwordResetStorage.get(blApiRequest.documentId).then((passwordReset: PasswordReset) => {
				this._localLoginHandler.setPassword(passwordReset.email, newPassword).then(() => {
					this._resHandler.sendResponse(res, new BlapiResponse([{success: true}]));

					resolve(true);
				}).catch((setPasswordError: BlError) => {
					reject(new BlError('could not update localLogin with password').add(setPasswordError));
				});
