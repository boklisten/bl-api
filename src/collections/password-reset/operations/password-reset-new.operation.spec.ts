...
describe('PasswordResetNewOperation', () => {
	const passwordResetStorage = new BlDocumentStorage<PasswordReset>('passwordresets');
	const localLoginHandler = new LocalLoginHandler();
	const resHandler = new SEResponseHandler();
	const passwordResetNewOperation = new PasswordResetNewOperation(passwordResetStorage, localLoginHandler, resHandler);
	...
	let responseHandlerSpy = sinon.stub(resHandler, 'sendResponse').callsFake(() => {
		return true;
	});

	sinon.stub(localLoginHandler, 'setPassword').callsFake((username: string, password: string) => {
	...
	describe('#run', () => {
		it('should reject if blApiRequest.documentId is not found', () => {
			...
			return expect(passwordResetNewOperation.run(testBlApiRequest))
				.to.be.rejectedWith(BlError, /passwordReset "notFoundPasswordReset" not found/);
		});
		...
		it('should reject if blApiRequest.data.password is null or undefined', () => {
			...
			return expect(passwordResetNewOperation.run(testBlApiRequest))
				.to.be.rejectedWith(BlError, /blApiRequest.data.password is null or undefined/);
		});
		...
		it('should reject if blApiRequest.data.password is under length of 6', () => {
			...
			return expect(passwordResetNewOperation.run(testBlApiRequest))
				.to.be.rejectedWith(BlError, /blApiRequest.data.password is under length of 6/);
		});
