...
describe('PasswordResetOperation', () => {
	...
	const passwordResetOperation = new PasswordResetOperation(passwordResetStorage);
	...
	beforeEach(() => {
		...
		testRedirectPath = process.env.CLIENT_URI + 'auth/password/' + testPasswordReset.id + '/reset';
	});
	...
	describe('#run', () => {
		it('should reject if documentId is not found', () => {
			...
			return expect(passwordResetOperation.run(testBlApiRequest))
		...
		it('should redirect to correct path if passwordReset is found', (done) => {
			...
			passwordResetOperation.run(testBlApiRequest, null, testResponse).then(() => {
				expect(testResponseRedirectSpy.calledOnce)
					.to.be.true;

				expect(testResponseRedirectSpy.calledWith(testRedirectPath))
					.to.be.true;
