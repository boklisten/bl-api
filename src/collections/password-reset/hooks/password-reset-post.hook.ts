...
export class PasswordResetPostHook extends Hook {
	...
	constructor(userDetailStorage?: BlDocumentStorage<UserDetail>, userHandler?: UserHandler, seCrypto?: SeCrypto, messenger?: Messenger) {
		super();
	...
	before(passwordReset: PasswordReset, accessToken?: AccessToken): Promise<PasswordReset> {
		if (isNullOrUndefined(passwordReset)) {
			return Promise.reject(new BlError('passwordReset is empty or undefined'))
		}

		if (isNullOrUndefined(passwordReset.email) || !isEmail(passwordReset.email)) {
			return Promise.reject(new BlError(`passwordReset.email is not a valid email`))
		}
		...
		return new Promise((resolve, reject) => {
			this._userHandler.getByUsername(passwordReset.email).then((user: User) => {
				...
				reject(new BlError(`user "${passwordReset.email}" not found`).add(getUserError));
			})
		});
	}

	after(passwordResets: PasswordReset[], accessToken?: AccessToken): Promise<PasswordReset[]> {
		return new Promise((resolve, reject) => {
			...
			this._userDetailStorage.get(passwordReset.userDetail).then((userDetail: UserDetail) => {
				...
				reject(new BlError(`userDetail "${passwordReset.userDetail}" not found`).add(getUserDetailError));
			});
		});
	}
}