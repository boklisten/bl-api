...
export class PaymentHandler {
  ...
  public async confirmPayments(
    ...
    if (payments.length > 1) {
      ...
      let updatedPayment = await this.paymentStorage.update(
        payments[0].id,
        {confirmed: true},
        {
          id: accessToken.sub,
  ...
  private async confirmMultiplePayments(
    order: Order,
    payments: Payment[],
    accessToken: AccessToken,
  ): Promise<boolean> {
      ...
      for (let payment of payments) {
        if (payment.method === 'dibs') {
          return Promise.reject(
            new BlError(
              `there was multiple payments but only one is allowed if one has method "${
                payment.method
              }"`,
            ),
          );
  ...
  private confirmBasedOnPaymentMethod(
    order: Order,
    payment: Payment,
    accessToken: AccessToken,
  ): Promise<boolean> {
  ...
  private confirmMethodDibs(
    ...
    return new Promise((resolve, reject) => {
      if (isNullOrUndefined(payment.info)) {
        return reject(
          new BlError('payment.method is "dibs" but payment.info is undefined'),
        );
      }
      ...
      this.dibsPaymentService
        .fetchDibsPaymentData(payment.info['paymentId'])
        .then((dibsEasyPayment: DibsEasyPayment) => {
          ...
          if (isNullOrUndefined(dibsEasyPayment.summary)) {
            reject(
              new BlError('dibsEasyPayment.summary is undefined').store(
                'dibsEasyPayment',
                dibsEasyPayment,
              ),
            );
          ...
          this.paymentStorage
            ...
            .catch((updatePaymentError: BlError) => {
              reject(
                new BlError(
                  'could not update payment with DibsEasyPayment',
                ).add(updatePaymentError),
              );
            });
        })
        .catch((getDibsPaymentError: BlError) => {
          reject(
            new BlError('could not get dibs payment on dibs api').add(
              getDibsPaymentError,
            ),
          );
        });
