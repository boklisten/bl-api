...
describe('PaymentDibsHandler', () => {
	...
	describe('handleDibsPayment()', () => {
		...
		sinon.stub(dibsPaymentService, 'orderToDibsEasyOrder').callsFake((order: Order) => {
			return (getDibsEasyOrderConfirm) ? Promise.resolve(testDibsEasyOrder) : Promise.reject(new BlError('could not create dibs easy order'));
		});
		
		sinon.stub(dibsPaymentService, 'getPaymentId').callsFake((dibsEasyOrder: DibsEasyOrder) => {
			return (getPaymentIdConfirm) ? Promise.resolve(testPaymentId) : Promise.reject(new BlError('could not create paymentId'));
		});
		...
		sinon.stub(orderStorage, 'get').callsFake((id: string) => {
			return (id === testOrder.id) ? Promise.resolve(testOrder) : Promise.reject(new BlError('order not found'));
		});
