...
import {BlError, Delivery, Order, Branch} from '@wizardcoder/bl-model';
...
describe('DeliveryValidator', () => {
	...
	const deliveryValidator = new DeliveryValidator(orderStorage, deliveryBranchHandler, deliveryBringHandler);
	...
	sinon.stub(deliveryBranchHandler, 'validate').callsFake((delivery: Delivery, order: Order) => {
	...
	sinon.stub(deliveryBringHandler, 'validate').callsFake((delivery: Delivery, order: Order) => {
	...
	describe('validate()', () => {
		...
		it('should reject with error when method is not defined', () => {
			...
			return expect(deliveryValidator.validate(testDelivery, testOrder))
		...
		it('should reject if delivery.method is branch and DeliveryBranchHandler.validate rejects', () => {
			...
			return expect(deliveryValidator.validate(testDelivery, testOrder))
				.to.be.rejectedWith(BlError, /validation of delivery.method "branch" failed/);
		});
		...
		it('should reject if delivery.method is bring and DeliveryBringHandler.validate rejects', () => {
			...
			return expect(deliveryValidator.validate(testDelivery, testOrder))
				.to.be.rejectedWith(BlError, /validation of delivery.method "bring" failed/);
		});
	});
});