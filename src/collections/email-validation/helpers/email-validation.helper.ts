...
export class EmailValidationHelper {
	...
	constructor(messenger?: Messenger, userDetailStorage?: BlDocumentStorage<UserDetail>,
	...
	public createAndSendEmailValidationLink(userDetailId: string): Promise<boolean> {
		return new Promise((resolve, reject) => {
			this._userDetailStorage.get(userDetailId).then((userDetail: UserDetail) => {
				let emailValidation = {
					email: userDetail.email,
					userDetail: userDetail.id
				} as EmailValidation;

				this._emailValidationStorage.add(emailValidation, new SystemUser()).then((addedEmailValidation: EmailValidation) => {

					this._messenger.emailConfirmation(userDetail, addedEmailValidation.id);
