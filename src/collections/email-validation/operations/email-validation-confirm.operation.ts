...
export class EmailValidationConfirmOperation implements Operation {
	...
	constructor(emailValidationStorage?: BlDocumentStorage<EmailValidation>, resHandler?: SEResponseHandler,
				userDetailStorage?: BlDocumentStorage<UserDetail>) {
		this._emailValidationStorage = (emailValidationStorage) ? emailValidationStorage : new BlDocumentStorage('email_validations', emailValidationSchema);
	...
	run(blApiRequest: BlApiRequest, req?: Request, res?: Response, next?: NextFunction): Promise<boolean> {
		return new Promise((resolve, reject) => {
			...
			this._emailValidationStorage.get(blApiRequest.documentId).then((emailValidation: EmailValidation) => {

					this._userDetailStorage.update(emailValidation.userDetail, {emailConfirmed: true}, new SystemUser()).then(() => {
						this._resHandler.sendResponse(res, new BlapiResponse([{confirmed: true}]));
						resolve(true);
					}).catch((updateUserDetailError: BlError) => {
						let err = new BlError(`could not update userDetail "${emailValidation.id}" with emailConfirmed true`).add(updateUserDetailError);
				...
				reject(new BlError(`emailValidation "${blApiRequest.documentId}" not found`).code(702).add(getEmailValidationError));
			});
