  ...
  public add(key: string, value: any): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.redisClient.set(key, value, (error, reply) => {
        if (error || reply === null)
          reject(
            new BlError('could not insert value with "' + key + '"')
  ...
  public get(key: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.redisClient.get(key, (error, value) => {
        if (error || value === null)
          reject(
            new BlError('could not get value with key "' + key + '"')
              .className("RedisHandler")
              .methodName("get")
              .code(702)
          );
  ...
  public remove(key: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.redisClient.del(key, (error, reply) => {
        if (error || reply === null)
          reject(
            new BlError('could not delete value with key "' + key + '"')
              .className("RedisHandler")
              .methodName("remove")
              .code(702)
          );
