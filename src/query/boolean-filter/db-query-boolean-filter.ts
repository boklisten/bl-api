...
export class DbQueryBooleanFilter {
	...
	public getBooleanFilters(query: any, validBooleanParams: string[]): BooleanFilter[] {
	...
	private generateBooleanFilters(query: any, validBooleanParams: string[]): BooleanFilter[] {
		...
		for (let param in query) {
			if (validBooleanParams.indexOf(param) > -1) {
				...
				if (query[param] === 'true') {
					...
					throw new TypeError('value "' + query[param] + '" could not be parsed to boolean');
				}
