...
describe('DbQueryBooleanFilter', () => {
	...
	describe('getBooleanFilters', () => {
		...
		it('should return array of [{fieldName: "haveEaten", value: true"}]', () => {
			...
			expect(dbQueryBooleanFilter.getBooleanFilters({haveEaten: 'true'}, ['haveEaten']))
				.to.eql(result);
		});
		...
		it('should return array that includes all params that are of boolean type in query', () => {
			...
			expect(dbQueryBooleanFilter.getBooleanFilters({confirmed: 'true', hasCar: 'false', isOld: 'true'},
				['confirmed', 'hasCar', 'isOld', 'haveChildren'])).to.eql(result);
		});
	});
});