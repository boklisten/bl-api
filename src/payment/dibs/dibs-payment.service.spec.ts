...
describe('DibsPaymentService', () => {
	...
	const dibsPaymentService: DibsPaymentService = new DibsPaymentService(deliveryStorage, httpHandler);
	let testOrder: Order;
	...
	describe('#orderToDibsEasyOrder', () => {
		...
		it('should throw error if order.id is not defined', () => {
			...
			expect(() => {
				dibsPaymentService.orderToDibsEasyOrder(testOrder);
		...
		it('should return a total amount of 10000 when item costs 100kr', () => {
			...
			let deo: DibsEasyOrder = dibsPaymentService.orderToDibsEasyOrder(testOrder);
		...
		context('dibsEasyOrder.items should be valid', () => {
			...
			it('should have name of "signatur 3"', () => {
				...
				let deo = dibsPaymentService.orderToDibsEasyOrder(testOrder);
	...
	sinon.stub(httpHandler, 'get').callsFake((url: string, authorization?: string) => {
	...
	describe('#fetchDibsPaymentData', () => {
		it('should reject if httpHandler rejects', () => {
			...
			return expect(dibsPaymentService.fetchDibsPaymentData('dibsPaymentId1'))
				.to.be.rejectedWith(BlError, /could not get payment details for paymentId "dibsPaymentId1"/);
		});
		...
		it('should reject if dibsResponse does not include a payment', (done) => {
			...
			dibsPaymentService.fetchDibsPaymentData('dibsPaymentId1').catch((err: BlError) => {
				expect(err.errorStack[0].getMsg())
					.to.be.eq('dibs response did not include payment information');
				done();
			})
		});
		...
		it('should resolve with a dibsEasyPayment object with correct paymentId', () => {
			...
			return expect(dibsPaymentService.fetchDibsPaymentData('aPaymentId'))
