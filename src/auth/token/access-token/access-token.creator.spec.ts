	...
	describe('createAccessToken()', () => {
		...
		context('when parameter is malformed', () => {
			it('should reject with BlError when username is undefined', () => {
				...
				return accessTokenCreator.create(username, testUserid, testPermission, testUserDetailId, testRefreshToken)
			...
			it('should reject with BlError when userId is undefined', () => {
				...
				return accessTokenCreator.create(testUsername, userid, testPermission, testUserDetailId, testRefreshToken)
					.should.be.rejectedWith(BlError);
			});
			
			it('should should reject with BlError when requestToken is undefined', () => {
				let refreshToken = '';
				return accessTokenCreator.create(testUsername, testUserid, testPermission, testUserDetailId, refreshToken)
					.should.be.rejectedWith(BlError);
			});
		});
		...
		context('when refreshToken is not valid', () => {
			it('should reject with BlError code 905', (done) => {
				...
				accessTokenCreator.create(testUsername, testUserid, testPermission, testUserDetailId, refreshToken).then(
					(accessToken: string) => {
		...
		context('when all parameters is valid', () => {
			it('should resolve with a accessToken', (done) => {
				accessTokenCreator.create(testUsername, testUserid, testPermission, testUserDetailId, testRefreshToken).then(
					(accessToken: string) => {
